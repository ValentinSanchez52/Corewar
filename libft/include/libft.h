...
/*   Updated: 2019/08/19 21:21:16 by mbeilles         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void					*push_to_mem_block(t_mem_block *block, void *data,
		size_t size);
void					*store_to_mem_block(t_mem_block *block, void *d,
		size_t size);
void					*allocate_elem_on_mem_block(t_mem_block *block,
		uint32_t size);
void					*force_allocate_elem_on_mem_block(t_mem_block *b,
		uint32_t s);
void					remove_elem_of_mem_block(t_mem_block *b, void *d,
		uint32_t s);
...
char					*ft_ultostr(uint64_t num, uint8_t radix,
		uint8_t upcase);
char					*ft_ltostr(int64_t num, uint8_t radix,
		uint8_t upcase);
char					*ft_dtostr(double d, uint32_t precision,
		bool force_sign);
...
char					*ft_strajoin(int len, ...);
char					*ft_strjoin_free(char const *s1, char const *s2);

char					*ft_strchr(const char *str, int c);
char					*ft_strrchr(char const *s, int c);
char					*ft_strreplace(char const *src, char const *pat,
		char const *r);

char					*ft_strskip(char *s, int (*cmp)(int c));
...
NORETURN				ft_exit(int code, char *message,
		void (*callback)(void*));
...
int32_t					ft_error(uint32_t n, char **str,
		int32_t return_status);
NORETURN				ft_error_exit(uint32_t n, char **str,
		int32_t exit_status);
