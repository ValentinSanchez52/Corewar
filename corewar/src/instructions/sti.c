...
/*   Updated: 2019/07/26 06:22:05 by vsanchez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				sti(t_op *op)
{
	...
	if (op->types[1] == COR_ARG_REG)
		...
		value_1 = get_mem_value(op->process->pc + ((uint16_t)op->args[1]
					% COR_IDX_MOD), COR_REG_SIZE);
	...
	set_mem_value((value_1 + value_2) % COR_IDX_MOD, op->args[0], COR_REG_SIZE);
}
