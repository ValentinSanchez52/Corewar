...
/*   Updated: 2019/08/19 19:39:24 by mbeilles         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline void	load_warriors_in_arena(void)
{
	...
	while (warrior_i < 4 && spawn_i < 4)
	{
		...
		{
			ft_memcpy(&(g_vm.arena[spawn_i * COR_ARENA_SIZE
						/ g_vm.warriors_nb]),
					g_vm.warriors[warrior_i].assembly,
					g_vm.warriors[warrior_i].assembly_size);
			ft_memset(&g_vm.arena_claim[spawn_i * COR_ARENA_SIZE
					/ g_vm.warriors_nb], warrior_i + 1,
					g_vm.warriors[warrior_i].assembly_size);
			process = (t_process){.registers[0] = g_vm.warriors[warrior_i].id,
					.global_offset = spawn_i * COR_ARENA_SIZE
						/ g_vm.warriors_nb};
